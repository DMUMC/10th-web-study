...
function useGetLpList({ cursor, search, order, limit }: PaginationDto) {
    return useQuery({
        ...
        // 컴포넌트가 마운터 되거나 창에 포커스 들어오는 경우도 재요청 X
        // 5분 동안 기존 데이터를 그대로 활용해서 네트워크 요청을 줄인다.
        staleTime: 1000 * 60 * 5,//5분

        //사용되지 않는 (비활성 상태) 인 쿼리 데이터가 캐시에 남아있는 시간
        // statleTime이 지나고 데이터가 신선하지 않더라도, 일정 시간 동안 메모리에 보관.
        ...
        //컴포넌트가 렌더링 될 때 반 데이터 구조를 미리 제공해서, 로딩 전에도 안전하게 UI를 구성할 수 있게 해주는애다.
        // initialData: initialLpListData,

        //파라미터가 변경될 떄 이전 데이터를 유지하여 UI 깜빡임을 (Flicking)을 줄여줍니다.
