...
const UserInput = ({ error, touched, ...props}: UserInputProps) => {
  return (
    <div className="w-full mb-4">
      <input
        {...props}
        className={`w-full p-4 bg-white/5 border rounded-xl text-white placeholder:text-gray-500 focus:outline-none transition-all
          ${error && touched 
            ? 'border-red-500 focus:border-red-500' 
            : 'border-white/10 focus:border-[#FF007A] focus:bg-white/10'
          }`}
      />
      {error && touched && (
