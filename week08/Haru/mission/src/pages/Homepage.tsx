...
const HomePage = () => {
  ...
  const {
    ...
    isError,
    refetch
  } = useGetLpInfiniteList(order, searchQuery);

  const throttledFetchTrigger = useThrottle(inView && hasNextPage && !isFetchingNextPage, 1500);
  ...
  if (isLoading) {
    return (
      <div className="space-y-8">
        <LpSearch onSearchChange={handleSearchChange} />
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-center">
        {searchQuery.trim() !== "" ? (
          <h3 className="text-sm font-semibold text-gray-400">"{searchQuery}" 검색 결과</h3>
        ) : (
          <div />
        )}
     ...
     <div ref={ref} className="py-10">
      ...
      {data?.pages[0]?.data.length === 0 && (
        <div className="py-20 text-center text-gray-500">
          {searchQuery.trim() !== "" ? "검색 결과에 맞는 LP가 없습니다." : "등록된 LP가 없습니다."}
        </div>
      )}
    </div>
  );
