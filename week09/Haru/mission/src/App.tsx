...
function App() {
  const { cartItems, amount, total, increase, decrease, removeItem, calculateTotals } = useCartStore();
          ...
          <div key={item.id} className="flex items-center gap-4 bg-white p-4 rounded shadow">
            ...
            <div className="flex-1">
              ...
              <div className="mt-2 flex gap-2">
                <button onClick={() => increase(item.id)} className="px-2 bg-gray-200">+</button>
                <span>{item.amount}</span>
                <button onClick={() => decrease(item.id)} className="px-2 bg-gray-200">-</button>
                <button onClick={() => removeItem(item.id)} className="text-red-500 text-sm ml-4">삭제</button>
              </div>
            </div>
          </div>
        ))}
